...
#To drop columns which are useless/insignificant
data1[ ,`:=`(RowNumber = NULL, Surname = NULL, CustomerID = NULL,
             LastTransactionAmt = NULL,Retention = NULL, CLV = NULL,
             Discount = NULL)] 
...
risk <- ggplot(data1, aes(RiskProfile)) +
  geom_density(color = 'black', fill="gray") + 
  geom_vline(aes(xintercept=mean(PortfolioReturn)),color="blue", linetype="dashed", size=1) +
