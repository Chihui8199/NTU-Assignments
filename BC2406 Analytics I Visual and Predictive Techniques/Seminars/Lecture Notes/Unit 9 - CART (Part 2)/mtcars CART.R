...
#Ans: Maybe not the best way to do this haha x
df  <- cart2$variable.importance
a  = as.data.frame(df)
a[,]/sum(a) * 100
...
default<- fread("default.csv", stringsAsFactors = T)
set.seed(2014)
# Continuous Y: Set method = 'class'
cart1 <- rpart(Default ~ ., data = default, method = 'class', control = rpart.control(minsplit = 2, cp = 0))
printcp(cart1)
## Caution: printcp() shows that if you forgot to change the default CP from 0.01 to 0,
## It would have stopped the tree growing process too early. A lot of further growth at CP < 0.01.
plotcp(cart1)
print(cart1) #Difficult to see which tree is optimal
